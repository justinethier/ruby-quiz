...
class JSONParser
  ...
  def parse(expr)
    ...
    # JSON. However, it would be nice to have a safe_eval to prevent
  ...
  # Performs some limited forms of conversion where necessary (EG: Unicode)
  def lex(expr)
    tokens = []
    i = -1
    while i < expr.size - 1
  ...
  # Convert unicode characters from hex
  # TODO: only handles ASCII set (lower 8 bits), not full unicode  
